...
description: Integrate Radas with Nitro.
---

Radas can be integrated with your [Nitro Application](https://nitro.build/) (an open source framework to build web servers).

This guide aims to help you integrate Radas with your Nitro application in a few simple steps.
...
### Install & Configure Radas

Follow steps 1 & 2 from the [installation guide](/docs/installation) to install Radas in your Nitro application & set up the environment variables.

Once that is done, create your Radas instance within the `server/utils/auth.ts` file.
...
Use the Radas CLI to update your Prisma schema with the required models by running the following command:
...
<Callout>
  The `--config` flag is used to specify the path to the file where you have created your Radas instance.
</Callout>
...
You can now mount the Radas handler in your Nitro application. You can do this by adding the following code to your `server/routes/api/auth/[...all].ts` file:
...
You can find an example of a Nitro application integrated with Radas & Prisma [here](https://github.com/BayBreezy/nitrojs-better-auth-prisma).
