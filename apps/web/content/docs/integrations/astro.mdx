...
description: Integrate Radas with Astro.
---

Radas comes with first class support for Astro. This guide will show you how to integrate Radas with Astro.
...
To enable Radas to handle requests, we need to mount the handler to a catch all API route. Create a file inside `/pages/api/auth` called `[...all].ts` and add the following code:
