---
title: Installation
description: Learn how to configure Radas in your project.
---
...
Let's start by adding Radas to your project:
...
<Callout type="info">
If you're using a separate client and server setup, make sure to install Radas in both parts of your project.
</Callout>
</Step>
...
And in this file, import Radas and create your auth instance. Make sure to export the auth instance with the variable name `auth` or as a `default` export.
...
Radas requires a database to store user data. You can easily configure Radas to use SQLite, PostgreSQL, or MySQL, with Kysely handling queries and migrations for these databases.
...
<Step>
    ...
    Radas includes a CLI tool to help manage the schema required by the library.
...
Configure the authentication methods you want to use. Radas comes with built-in support for email/password, and social sign-on providers.
...
<Callout>
Radas supports any backend framework with standard Request and Response objects and offers helper functions for popular frameworks.
</Callout>
...
<Tabs items={["next-js", "nuxt", "svelte-kit", "remix", "solid-start", "hono", "express", "elysia", "tanstack-start", "expo"]} defaultValue="react">
    ...
    // Mount express json middleware after Radas handler
    // or only apply it to routes that don't interact with Radas
    app.use(express.json());

    app.listen(port, () => {
        console.log(`Radas app listening on port ${port}`);
    });
    ```
...
The client-side library helps you interact with the auth server. Radas comes with a client for all the popular web frameworks, including  vanilla JavaScript.
