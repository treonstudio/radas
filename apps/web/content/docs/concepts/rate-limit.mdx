...
Radas includes a built-in rate limiter to help manage traffic and prevent abuse. By default, in production mode, the rate limiter is set to:
...
In addition to the default settings, Radas provides custom rules for specific paths. For example:
...
When a request exceeds the rate limit, Radas returns the following header:

- `X-Retry-After`: The number of seconds until the user can make another request.

To handle rate limit errors on the client side, you can manage them either globally or on a per-request basis. Since Radas clients wrap over Better Fetch, you can pass `fetchOptions` to handle rate limit errors
