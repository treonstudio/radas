---
title: API
description: Radas API.
---

When you create a new Radas instance, it provides you with an `api` object. This object exposes every endpoint that exist in your Radas instance. And you can use this to interact with Radas server side.

Any endpoint added to Radas, whether from plugins or the core, will be accessible through the `api` object.
...
<Callout>
Radas API endpoints are built on top of [better-call](https://github.com/bekacru/better-call), a tiny web framework that lets you call REST API endpoints as if they were regular functions and allows us to easily infer client types from the server.
</Callout>
