---
title: Cookies
description: Learn how cookies are used in Radas.
---
...
Radas cookies will follow `${prefix}.${cookie_name}` format by default. The prefix will be "better-auth" by default. You can change the prefix by setting `cookiePrefix` in the `advanced` object of the auth options.
...
By default, Radas uses the following cookies:
