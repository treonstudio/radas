---
title: Options
description: Radas configuration options reference.
---

List of all the available options for configuring Radas. See [Radas Options](https://github.com/better-auth/better-auth/blob/main/packages/better-auth/src/types/options.ts#L13).
...
Base URL for Radas. This is typically the root URL where your application server is hosted. Note: If you include a path in the baseURL, it will take precedence over the default path.
...
Base path for Radas. This is typically the path where the Radas routes are mounted. It will be overridden if there is a path component within `baseURL`.
...
By default, Radas will look for the following environment variables:
...
Database configuration for Radas.
...
Radas supports various database configurations including [PostgreSQL](/docs/adapters/postgresql), [MySQL](/docs/adapters/mysql), and [SQLite](/docs/adapters/sqlite).
