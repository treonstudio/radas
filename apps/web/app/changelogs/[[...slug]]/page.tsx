...
export async function generateMetadata({
	...
	let baseUrl = process.env.NEXT_PUBLIC_URL || process.env.VERCEL_URL;
	if (!baseUrl) {
	...
	let url: URL;
	try {
		url = new URL(`${baseUrl}/release-og/${slug.join("")}.png`);
	} catch (e) {
		url = new URL(`http://localhost:3000/release-og/${slug.join("")}.png`);
	}
